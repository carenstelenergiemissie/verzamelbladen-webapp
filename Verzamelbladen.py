...
"""
Verzamelblad Generator - Verbeterde versie met werkende PDF export
"""
import streamlit as st
import pandas as pd
import os
...
# Nieuwe imports voor PDF generatie
try:
...
# =====================================================================
# PDF GENERATIE FUNCTIE - Platformonafhankelijk
# =====================================================================
def excel_to_pdf_reportlab(excel_bytes, sheet_name="Verzamelblad"):
    """
    Converteer een Excel verzamelblad naar PDF met ReportLab
    """
    ...
    try:
        ...
        # Lees belangrijke cellen voor header
        klant = ws['B3'].value or ""
        datum = ws['B4'].value or ""
        periode = ws['C26'].value or ""
        
        # Titel
        story.append(Paragraph(f"<b>Verzamelblad - {klant}</b>", title_style))
        ...
        # Verzamelblad data - probeer eerst standaard locatie
        # Meestal begint de tabel bij rij 7 of 8
        data_start_row = None
        for row_num in range(6, 15):  # Zoek tussen rij 6 en 15
            cell_val = ws.cell(row_num, 1).value
            if cell_val and isinstance(cell_val, str) and ('EAN' in cell_val.upper() or 'NUMMER' in cell_val.upper()):
        ...
        if data_start_row:
            ...
            col = 1
            while True:
                val = ws.cell(data_start_row, col).value
                if val is None:
                    break
            ...
            while True:
                ...
                # Veiligheid: stop na 100 rijen
                if row > data_start_row + 100:
                    break
            ...
            if len(data_rows) > 1:
                ...
                available_width = 18 * cm  # A4 breedte minus marges
                col_widths = [available_width / num_cols] * num_cols
                
                # Pas kolom breedtes aan voor specifieke kolommen
                if num_cols >= 3:
                    # Maak bedrag kolommen smaller, rest wider
                    for i in range(num_cols):
                        if i < 2:  # Eerste 2 kolommen (EAN, Factuurnummer)
                            col_widths[i] = available_width * 0.2
                        else:  # Bedrag kolommen
                            col_widths[i] = available_width * 0.6 / (num_cols - 2)
                ...
                table_style = [
                    ...
                    ('FONTSIZE', (0, 1), (-1, -1), 8),
                    ('ALIGN', (0, 1), (1, -1), 'LEFT'),  # Eerste 2 kolommen links
                    ('ALIGN', (2, 1), (-1, -1), 'RIGHT'),  # Rest rechts
        ...
        # Totalen (uit cellen E27, F27, G27 of vergelijkbaar)
        # Zoek naar "Totaal" rij
        totaal_row = None
        for row_num in range(20, 35):
            cell_val = ws.cell(row_num, 1).value
            if cell_val and isinstance(cell_val, str) and 'TOTAAL' in cell_val.upper():
                totaal_row = row_num
                break
        ...
        if totaal_row:
            totaal_data = [
                ...
                [
                    "Totaal:",
                    f"€ {ws.cell(totaal_row, 5).value or 0:,.2f}",
                    f"€ {ws.cell(totaal_row, 6).value or 0:,.2f}",
                    f"€ {ws.cell(totaal_row, 7).value or 0:,.2f}"
                ]
            ]
            
            totaal_table = Table(totaal_data, colWidths=[4*cm, 4*cm, 4*cm, 4*cm])
            totaal_table.setStyle(TableStyle([
                ('BACKGROUND', (1, 0), (-1, 0), colors.HexColor('#667eea')),
                ...
                ('GRID', (1, 0), (-1, -1), 1, colors.HexColor('#667eea')),
                ('BACKGROUND', (1, 1), (-1, 1), colors.HexColor('#f0fdf4')),
                ('TOPPADDING', (0, 0), (-1, -1), 8),
                ('BOTTOMPADDING', (0, 0), (-1, -1), 8),
            ]))
            
            story.append(totaal_table)
...
def process_supplier(
    ...
    try:
        ...
        # PDF generatie - NIEUWE METHODE
        pdf_bytes = None
        pdf_filename = None
        
        if bedragen_kloppen and PDF_AVAILABLE:
            try:
                pdf_bytes = excel_to_pdf_reportlab(excel_bytes, "Verzamelblad")
                if pdf_bytes:
                    pdf_filename = output_filename.replace('.xlsx', '.pdf')
            except Exception as e:
                print(f"PDF generatie fout: {e}")
...
def main():
    ...
    if not PDF_AVAILABLE:
        st.warning("⚠️ PDF generatie niet beschikbaar. Installeer: `pip install reportlab`")
    ...
    with st.sidebar:
        ...
        if PDF_AVAILABLE:
            st.success("✅ PDF generatie: Actief")
        else:
            st.error("❌ PDF generatie: Niet beschikbaar")
                    ...
                    with st.container():
                        ...
                        with col_pdf:
                            if result.get("pdf_bytes"):
                                ...
                                st.info("📄 PDF: Generatie mislukt", icon="⚠️")
